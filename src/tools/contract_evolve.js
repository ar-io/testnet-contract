(() => {
  ...
  var buyRecord = async (state, { caller, input: { name, contractTxId, years, tier } }) => {
    const balances = state.balances;
    const records = state.records;
    ...
    if (!Number.isInteger(years) || years > MAX_YEARS || years <= 0) {
      throw new ContractError('Invalid value for "years". Must be an integer greater than zero and less than the max years');
    }
    ...
    if (name.charAt(0) === "-" || typeof name !== "string" || name.length > MAX_NAME_LENGTH || !nameRes || name === "www" || name === "") {
  ...
  var extendRecord = async (state, { caller, input: { name, years } }) => {
    ...
    if (records[name].endTimestamp + SECONDS_IN_GRACE_PERIOD < currentBlockTime) {
      throw new ContractError(`This name's lease has expired.  It must be purchased first before being extended.`);
    }
    let qty = fees[name.length.toString()] * records[name].tier * years;
    if (balances[caller] < qty) {
      throw new ContractError(`Caller balance not high enough to extend this name lease for ${qty} token(s) for ${years}!`);
    }
    balances[caller] -= qty;
    state.foundation.balance += Math.floor(qty * (FOUNDATION_PERCENTAGE / 100));
    state.rewards += Math.floor(qty * ((100 - FOUNDATION_PERCENTAGE) / 100));
  ...
  var upgradeTier = async (state, { caller, input: { name, tier } }) => {
    ...
    let qty = Math.ceil(fees[name.length.toString()] * levelsUpgraded * amountOfYearsLeft);
    if (balances[caller] < qty) {
      throw new ContractError(`Caller balance not high enough to extend this name lease for ${qty} token(s)!`);
    }
    balances[caller] -= qty;
    state.foundation.balance += Math.floor(qty * (FOUNDATION_PERCENTAGE / 100));
    state.rewards += Math.floor(qty * ((100 - FOUNDATION_PERCENTAGE) / 100));
  ...
  var setTier = async (state, { caller, input: { tier, maxSubdomains, minTtlSeconds } }) => {
    const owner = state.owner;
    if (caller !== owner) {
      throw new ContractError("Caller cannot change tiers");
    }
    if (!Number.isInteger(maxSubdomains) || !Number.isInteger(tier) || !Number.isInteger(minTtlSeconds)) {
  ...
  var lock = async (state, { caller, input: { qty, lockLength } }) => {
    ...
    if (!Number.isInteger(lockLength) || lockLength < settings["lockMinLength"] || lockLength > settings["lockMaxLength"]) {
      throw new ContractError(`lockLength is out of range. lockLength must be between ${settings["lockMinLength"]} - ${settings["lockMaxLength"]}.`);
    }
    if (!balances[caller] || balances[caller] == void 0 || balances[caller] == null || isNaN(balances[caller])) {
      throw new ContractError(`Caller balance is not defined!`);
    }
  ...
  var unlock = async (state, { caller, input: { id } }) => {
    ...
    if (id && (typeof id !== "number" || id >= vaults[caller].length && id < 0)) {
  ...
  var increaseVaultLength = async (state, { caller, input: { id, lockLength } }) => {
    const settings = state.settings;
    const vaults = state.vaults;
    if (!Number.isInteger(lockLength) || lockLength < settings["lockMinLength"] || lockLength > settings["lockMaxLength"]) {
      throw new ContractError(`lockLength is out of range. lockLength must be between ${settings["lockMinLength"]} - ${settings["lockMaxLength"]}.`);
  ...
  function isipV4Address(ipV4Address) {
    if (/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(ipV4Address)) {
      return true;
    }
  ...
  var transferTokensLocked = async (state, { caller, input: { target, qty, lockLength } }) => {
    ...
    if (balances[caller] < qty) {
      throw new ContractError(`Caller balance not high enough to send ${qty} locked token(s)!`);
    }
    if (!Number.isInteger(lockLength) || lockLength < settings["lockMinLength"] || lockLength > settings["lockMaxLength"]) {
      throw new ContractError(`lockLength is out of range. lockLength must be between ${settings["lockMinLength"]} - ${settings["lockMaxLength"]}.`);
    }
  ...
  var initiateFoundationAction = async (state, {
    caller,
    ...
    if (type === "transfer") {
      ...
      if (!Number.isInteger(qty) || qty <= 0 || qty > foundation.balance) {
        throw new ContractError('Invalid value for "qty". Must be a positive integer and must not be greater than the total balance available.');
      }
      if (lockLength) {
        if (!Number.isInteger(lockLength) || lockLength < settings["lockMinLength"] || lockLength > settings["lockMaxLength"]) {
          throw new ContractError(`lockLength is out of range. lockLength must be between ${settings["lockMinLength"]} - ${settings["lockMaxLength"]}.`);
      ...
      if (target in foundation.addresses) {
        throw new ContractError("Target is already added as a Foundation address");
      }
      ...
      if (!foundation.addresses.includes(target)) {
        throw new ContractError("Target is not in the list of Foundation addresses");
      }
    } else if (type === "setMinSignatures") {
      if (!Number.isInteger(value) || value <= 0 || value > foundation.addresses.length) {
        throw new ContractError("Invalid value for minSignatures. Must be a positive integer and must not be greater than the total number of addresses in the foundation.");
      }
    } else if (type === "setActionPeriod") {
      if (!Number.isInteger(value) || value <= 0) {
        throw new ContractError("Invalid value for transfer period. Must be a positive integer");
      }
    } else {
  ...
  var approveFoundationAction = async (state, { caller, input: { id } }) => {
    ...
    if (+SmartWeave.block.height >= action.start + foundation.actionPeriod && action.status === "active") {
    ...
    if (state.foundation.actions[id].totalSignatures >= foundation.minSignatures) {
  ...
  var joinNetwork = async (state, {
    ...
    if (balances[caller] < qty) {
      throw new ContractError(`Caller balance not high enough to stake ${qty} token(s)!`);
    }
    ...
    if (delegateAllowList) {
      if (!Array.isArray(delegateAllowList)) {
        throw new ContractError("Delegate allow list must contain arweave addresses.");
      }
    } else {
      delegateAllowList = [];
    }
  ...
  var leaveNetwork = async (state, { caller }) => {
    ...
    if (state.gateways[caller].vaults[0].start + settings.gatewayJoinLength > +SmartWeave.block.height) {
    ...
    if (state.gateways[caller].settings.url !== "") {
      for (let i = 0; i < state.gateways[caller].vaults.length; i++) {
        state.gateways[caller].vaults[i].end = +SmartWeave.block.height + settings.gatewayLeaveLength;
      }
      ...
      for (const key of Object.keys(state.gateways[caller].delegates)) {
        for (let i = 0; i < state.gateways[caller].delegates[key].length; i++) {
          ...
          state.gateways[caller].delegatedStake -= state.gateways[caller].delegates[key][i].balance;
  ...
  var delegateStake = async (state, { caller, input: { qty, target } }) => {
    ...
    if (target in gateways) {
      if (state.gateways[target].settings.openDelegation === false) {
        if (state.gateways[target].settings.delegateAllowList.indexOf(caller) <= -1) {
          throw new ContractError("This Gateway is not accepting non-allowed, community delegates");
        }
      }
  ...
  var undelegateStake = async (state, { caller, input: { id, target } }) => {
    ...
    if (!(caller in gateways[target].delegates && gateways[target].delegates[caller].length)) {
      throw new ContractError("No stake to undelegate");
    }
    if (id && (typeof id !== "number" || id >= gateways[target].delegates[caller].length && id < 0)) {
      throw new ContractError("Invalid vault index provided");
    }
    if (typeof id === "number" && id < gateways[target].delegates[caller].length && id >= 0) {
      if (gateways[target].delegates[caller][id].end === 0) {
        state.gateways[target].delegates[caller][id].end = +SmartWeave.block.height + settings.delegatedStakeWithdrawLength;
      } else if (gateways[target].delegates[caller][id].end <= +SmartWeave.block.height) {
        if (caller in state.balances) {
          state.balances[caller] += gateways[target].delegates[caller][id].balance;
        } else {
          state.balances[caller] = gateways[target].delegates[caller][id].balance;
        }
        state.gateways[target].delegatedStake -= state.gateways[target].delegates[caller][id].balance;
      ...
      for (let i = 0; i < gateways[target].delegates[caller].length; i++) {
        if (gateways[target].delegates[caller][i].end === 0) {
          state.gateways[target].delegates[caller][i].end = +SmartWeave.block.height + settings.delegatedStakeWithdrawLength;
        } else if (gateways[target].delegates[caller][i].end <= +SmartWeave.block.height) {
          if (caller in state.balances) {
            state.balances[caller] += gateways[target].delegates[caller][i].balance;
          } else {
            state.balances[caller] = gateways[target].delegates[caller][i].balance;
          }
          state.gateways[target].delegatedStake -= state.gateways[target].delegates[caller][i].balance;
  ...
  var decreaseOperatorStake = async (state, { caller, input: { id } }) => {
    ...
    if (id && (typeof id !== "number" || id >= gateways[caller].vaults.length && id < 0)) {
      throw new ContractError("Invalid vault index provided");
    }
    ...
    if (gateways[caller].vaults[id].end === 0) {
      state.gateways[caller].vaults[id].end = +SmartWeave.block.height + settings.operatorStakeWithdrawLength;
      ...
      state.gateways[caller].operatorStake -= state.gateways[caller].vaults[id].balance;
  ...
  var proposeGatewaySlash = async (state, { caller, input: { target, penalty } }) => {
    const settings = state.settings;
    const gateways = state.gateways;
    const owner = state.owner;
    if (caller !== owner) {
    ...
    if (state.gateways[target].operatorStake < settings.minGatewayStakeAmount) {
      ...
      for (const key of Object.keys(state.gateways[target].delegates)) {
        for (let i = 0; i < state.gateways[target].delegates[key].length; i++) {
          if (key in state.balances) {
            state.balances[key] += state.gateways[target].delegates[key][i].balance;
          } else {
            state.balances[key] = state.gateways[target].delegates[key][i].balance;
          }
          state.gateways[target].delegatedStake -= state.gateways[target].delegates[key][i].balance;
  ...
  var updateGatewaySettings = async (state, {
    ...
    if (caller in gateways) {
      ...
      if (sslFingerprint) {
        if (protocol !== "https") {
          throw new ContractError("This gateway must be set to HTTPS protocol first");
        } else {
