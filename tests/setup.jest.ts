...
import { createLocalWallet, mineBlock, setupInitialContractState } from "./utils/helper";
...
beforeAll(async () => {
    ...
    // create owner wallet
    const wallets = [await createLocalWallet(arweave), await createLocalWallet(arweave), await createLocalWallet(arweave)];
    const [owner] = wallets;

    // save wallets to disk
    wallets.forEach((w, index) => {
        fs.writeFileSync(path.join(__dirname, `./wallets/${index}`), JSON.stringify(w.wallet));
    })

    // // create initial contract
    const initialContractState = await setupInitialContractState(owner.address, wallets.map(w => w.address));
    ...
    fs.writeFileSync(path.join(__dirname, `./contract/arns_contract.json`), JSON.stringify({
