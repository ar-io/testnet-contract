...
import { ALLOWED_ACTIVE_TIERS, DEFAULT_NON_CONTRACT_OWNER_MESSAGE } from '../src/constants';
import {
    warp,
    arweave,
} from './setup.jest';
...
describe('Tiers', () => {
    ...
    describe('contract owner', () => {
        ...
        it('should not able to set active tier to an invalid tier number', async () => {
            ...
            expect(errors[writeInteraction!.originalTxId]).toEqual(`Invalid tier number provided. Allowed tier numbers: ${ALLOWED_ACTIVE_TIERS}`);
    ...
    describe('non-contract owner', () => {
        ...
        it('should not be able to create a new tier', async () => {
            ...
            const newState = newCachedValue.state as IOState;
            const errors = newCachedValue.errorMessages;
            expect(errors[writeInteraction!.originalTxId]).toEqual(DEFAULT_NON_CONTRACT_OWNER_MESSAGE);
    ...
    it('should be able to get active tiers via viewState', async () => {
        ...
        expect(activeTiers).toEqual(expect.arrayContaining(ALLOWED_ACTIVE_TIERS.map(_ => expectedTierObj)));
    });
})
